...
select * from emp where sal=max(sal);   -- 오류 발생
select * from emp where sal=(select max(sal) from emp);
-- 평균 월급 보다 많이 받아가는 사원의 정보
select * from emp where sal>avg(sal);   -- 오류 발생
select * from emp where sal>(select avg(sal) from emp);
-- 전체 사원의 수 = 전체 행수
select count(*) from emp;       -- 전체 행수     - 14개
select count(empno) from emp;   -- empno의 갯수  - 14개
select count(comm) from emp;    -- comm의 갯수   - 4개
...
-- 그룹핑 전의 조건 where
-- 그룹핑 후의 조건 having
-- 사원 중에서 부서별 평균 봉급이 300 이상인 부서와 사원의 평균 봉급은?
select deptno, avg(sal) from emp where job='사원' group by deptno having avg(sal)>=300;
-- 사원 중에서 부서별 평균 봉급이 300 이상인 부서와 사원의 평균 봉급 및 수는?
