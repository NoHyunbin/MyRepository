...
select * from emp where hiredate < '2005.01.01'; -- . - / 다 된다.
...
-- 입사날 내림차순 순서(1차정렬) -> 이름순(2차정렬)
select * from emp order by hiredate desc, ename asc;
-- 30부서에서 월급이 400 이상인 사원의 번호, 이름, 연봉(내림차순)을 가져오시오
...
-- 첫번째 미션
-- 1
select ename, sal, to_char(hiredate, 'MM/DD/YYYY') from emp;
