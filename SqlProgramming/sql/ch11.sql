...
-- 1.1 단일행 서브 쿼리 : 수행 결과가 오직 하나의 로우만을 반환하는 서브 쿼리
-- 이문세 사원의 부서 이름은?
select dname
from dept
where deptno=(select deptno from emp where ename='이문세');
...
-- Section03 서브쿼리를 이용하여 테이블을 생성하고 테이터를 조작하기
-- emp 테이블을 복제해서 emp2로 만드세요(하나의 DDL로 작성하시오)
-- 제약조건(PK , FK, Check, not null)은 복제가 되지 않습니다.
-- 인덱스도 복제가 되지 않습니다. 
create table emp2
as
select * from emp;

-- emp 테이블의 구조만 복제해서 emp3을 만드세요
...
-- 3.3 서브쿼리로 데이터를 삽입하기
-- emp3을 만들었다면 emp의 10번 부서의 사원들만 복사하세요.
insert into emp3
